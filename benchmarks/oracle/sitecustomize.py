...
if os.environ.get("COVERAGE_PROCESS_START"):
    ...
    class CachedAnalysis(Analysis):
        def __init__(self, *args, **kwargs) -> None:
            self.cache = {}
            # coverage.py calls branch helpers during Analysis.__init__ when
            # branch coverage is enabled. Initialize arc_data before super()
            # so those helpers can run before update_analysis populates it.
            self.arc_data = {}
        ...
        if regather_data:
            ...
            with data._connect() as con:
                ...
                with con.execute(
                    ...
                    arc_data = {}
                    for file_id, fromno, tono in cur:
                        arc_data.setdefault(file_id, []).append((fromno, tono))
    ...
    def get_lcov(self, fr, analysis, outfile=None):
        ...
        for block_number, block_line_number in enumerate(
            ...
            for branch_number, line_number in enumerate(
                ...
                outfile.write(
                    b"BRDA:%d,%d,%d,-\n"
                    % (line_number, block_number, branch_number)
                )
    ...
    lcov_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
